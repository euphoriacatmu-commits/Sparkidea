    ...
    while (retries > 0) {
      try {
        const res = await fetch('/api/ideation', {
          ...
          body: JSON.stringify({ userInput: text, modelSettings: settings }),
        })
        ...
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title="模型配置"
            >
              <span>{settings.provider === 'anthropic' ? '🤖' : settings.provider === 'openrouter' ? '🔀' : '🌋'}</span>
              <span>{PROVIDER_INFO[settings.provider].label}</span>
            </button>
            <button
              onClick={() => setShowModelConfig(true)}
